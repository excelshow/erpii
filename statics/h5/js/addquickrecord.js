var d;
function checkform() {
    var txtcarno = $(".fwadd_car dd:first")
        .find("a")
        .attr("carname");
    if ($.trim(txtcarno) == "") {
        Ewewo.Tips("请输入车牌!");
...
function getservicerecord(statusid) {
    ...
    servicerecord.id = 0;
    servicerecord.billtype = 1;
    servicerecord.starttime = $(".fwadd_car").attr("date");
    var isvip = $("div[name=customerinfodiv] li:first")
        .find("a")
        .attr("isvip");
    if (isvip == "1") {
    ...
    servicerecord.customermobile = $("div[name=customerinfodiv] li:first")
        .find("a")
        .attr("mobile")
        .replace("无手机号", "");
    servicerecord.carno = $(".fwadd_car dd:first")
        .find("a")
        .attr("carname");
    ...
    servicerecord.extension = { carprices: cardd.attr("price") || "" };
    ...
    servicerecord.serviceadvisor = $("#Workshilist a[name=aserviceadvisor]")
        .find("font")
        .text()
        .replace("请选择", "");
    ...
    servicerecord.outreport = $.trim($("#txtoutreport").val());
    ...
    if (customerid != "" || customerid > 0) {
        $.ajax({
            ...
            success: function (data) {
                $("#txtdiscount")
                    .attr("labourdiscount", data.LabourDiscount)
                    .attr("stockdiscount", data.StockDiscount);
            },
            error: function () {
    ...
    $("#workhour .workhour").each(function () {
        ...
        $(this)
            ...
            .each(function () {
                ...
                var childrentdtotal = childrenctr
                    .find("dd span[f=partstotal]")
                    .attr("itemid");
                var childrentdprojecttype = childrenctr
                    .find("dd span")
                    .eq(3)
                    .attr("projecttype");
                ...
                if (
                    childrenctr.attr("pid") != undefined &&
                    childrenctr.attr("pid") != "0"
                ) {
...
function getvippackage() {
    ...
    $("#packagetemp .taocansqlist").each(function () {
        ...
        p.servicename = $(this)
            .find("font[f=packagecar]")
            .attr("name");
        p.price = $(this)
            .find("font[f=packagecar]")
            .attr("price");
        p.servicecontent = $(this)
            .find("dd[f=packagecontent]")
            .attr("content");
        p.expirationdate = $(this)
            .find("font[f=packagecar]")
            .attr("effectivemoths")
            .replace("长期有效", "");
        p.salesemployeeid = $(this)
            .find("dd[f=sales]")
            .attr("salesid");
        packages.push(p);
    });
...
function getrechargeapplicaction() {
    ...
    $("#membershipcard .taocansqlist").each(function () {
        var item = {
            Id: 0,
            VipRechargeCardTemplateId: null,
            VipRechargeCardId: null,
            CardNo: "",
            Money: null,
        ...
        item.CardNo = $(this)
            ...
            .attr("cardno");
        item.Money = $(this)
            .find("dd[f=partstotal]")
            .attr("total");
        item.RealMoney = $(this)
            .find("dd[f=partstotal]")
            .attr("realmoney");
        item.IsOpenCard = false;
        item.SalesEmployeeId = $(this)
            .find("dd[f=sales]")
            .attr("salesid");
        recharge.push(item);
    });
...
$(function () {
    $(".fwadd_car dd:first,div[name=customerinfodiv] li:first").click(function () {
        ...
        maskshow(2);
        $(".mask").load(
            "/Service/ShowHtml?viewName=_SelectCarCustomer",
            {
                thisobj: json
            },
            function () {
    ...
    $(".fwadd_car dd")
        ...
        .click(function () {
            ...
            maskshow(2);
            $(".mask").load(
                "/Service/ShowHtml?viewName=_SelectCarModel",
                {
                    thisobj: json
                },
                function () {
    ...
    $(".fw_addmenu_left .add1")
        ...
        .click(function () {
            ...
            maskshow(2);
            $(".mask").load(
                "/Service/ShowHtml?viewName=_WorkingView",
                {
                    thisobj: json
                },
                function () {
    ...
    $(".fw_addmenu_left .add2")
        ...
        .click(function () {
            ...
            maskshow(2);
            $(".mask").load(
                "/Service/ShowHtml?viewName=_SelectPackage",
                {
                    thisobj: json
                },
                function () {
    ...
    /*
     * lixuan 2018-8-31 add 工时快速单增加常用工时选择
     */
    $(".fw_addmenu_left .add5")
        .find("a")
        .click(function () {
            var customerid = $("div[name=customerinfodiv] li:first")
                .find("a")
            ...
            maskshow(2);
            $(".mask").load(
                "/Service/ShowHtml?viewName=_WorkingCommonView",
                {
                    thisobj: json
                },
                function () {
    ...
    $("#fastworklist .addtcxm").click(function () {
        ...
        if (isshowwork == 0) {
            ...
            assemblyhtml(wid, randomid, total, salesname, salesid, operatername, operaterid, typename, name, qty, price, vipprice, servicetypeid, discount);
            updatebottomtotal();
        } else {
    ...
    $("#threeproject li")
        ...
        .click(function () {
            ...
            maskshow(2);
            $(".mask").load(
                "/Service/ShowHtml?viewName=_AdditionalProject",
                {
                    thisobj: json
                },
                function () {
    ...
    servicestatus();
    $(
        "#Workshilist a[name=achecker],#Workshilist a[name=aserviceadvisor]"
    ).click(function () {
        var qcstaff = $(this);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.result.length; i++) {
                        if (qcstaffid == data.result[i].id) {
                            html +=
                                ...
                                '" checkerid="' +
                                data.result[i].id +
                                '"><a href="javascript:void(0)"  class="carxz" >' +
                                data.result[i].name +
                                "&nbsp;-&nbsp;" +
                                data.result[i].no +
                                "</a></li>";
                    ...
                    $("#carchecker ul li").each(function () {
                        $(this).click(function () {
                            qcstaff
                                .attr("checkerid", $(this).attr("checkerid"))
                                .find("font")
                                .text($(this).attr("name"));
                            $(this)
                                .find("a")
    ...
    function carcustomersearchbykeyword(key, input) {
        ...
        if (key == keyword) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.result.length > 0) {
                        ...
                        for (var i = 0; i < data.result.length; i++) {
                            employeelist +=
                                "<li><a href='javascript:void(0)' customerid='" +
                                ...
                                data.result[i].carId +
                                "'  >" +
                                (data.result[i].mobile.length == 18
                                    ? "无手机号"
                                    : data.result[i].mobile) +
                                " - " +
                                data.result[i].name +
                        ...
                        $(input)
                            ...
                            .click(function (e) {
                                ...
                                $("#search_mobile")
                                    .val(mobile.length == 18 ? "无手机号" : mobile)
                                    .attr("selectid", customerid)
                                    .attr("mobile", mobile);
            ...
            $(".loading2").show();
            if (iscar == "1") {
                $(".search_fruit ul").html(
                    "<li><a href='javascript:void(0)' customerid='' carno='' nomobile=1 mobile='无手机号' style='text-align: center;'>无手机号</a></li>"
                );
                $(".search_fruit ul li a").click(function () {
                    if ($(this).attr("nomobile") == 1) {
                        ...
                        $("div[name=customerinfodiv] li:first")
                            .find("a")
                            .attr("mobile", $(this).attr("mobile"))
                            .attr("customerid", 0)
                            .find("font")
                            .html($(this).attr("mobile"));
                        maskshow(1);
                    }
                });
            } else {
    ...
    function getdata() {
        ...
        if (
            obj.mobile == "无手机号" &&
            $("#search_mobile").attr("mobile").length == 18
        ) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.result == "T") {
                    ...
                    window.location.href = "/Service/QuickDetail/" + data.id;
                } else {
                    masklayer(1, "保存中...");
...
function getcustomerinfo(carno, mobile, customerid) {
    $.ajax({
        type: "POST",
        data: { carno: carno, mobile: mobile, customerId: customerid },
            ...
            if (data.result == "T") {
                ...
                if (data.info.vip != null && data.info.vip == true) {
                    $("div[name=customerinfodiv] li:first")
                        ...
                        .attr("isvip", 1)
                        .find("font")
                        .html(mobile.length == 18 ? "无手机号" : mobile);
                } else {
                    $("div[name=customerinfodiv] li:first")
                        .find("a")
                        .attr("mobile", mobile)
                ...
                $("#txtcustomername")
                    .val(data.info.customerName)
                    .css("color", "#999");
                if (data.info.carId != 0) {
                    if (
                        data.info.maintainReport == null ||
                        data.info.maintainReport.AfterScore == null
                    ) {
                        $("#maintainreport")
                            .html("车况：<font>无</font>")
                            .attr(
                                "href",
                                "/carcheck/maintainreport?customerstoreitemid=" +
                                data.info.customerStoreItemId +
                                "&carid=" +
                                data.info.carId +
                                "&isnew=0&templateid=0&reportid=0&carno=" +
                                carno +
                                "&iswx=" +
                                (data.info.wxopenid == "" ? 0 : 1)
                            );
                    } else {
                        $("#maintainreport")
                            .html(
                                "车况：<font>" + data.info.maintainReport.AfterScore + "</font>"
                            )
                            .attr(
                                "href",
                                "/carcheck/maintainreport?customerstoreitemid=" +
                                ...
                                "&carid=" +
                                data.info.carId +
                                "&isnew=0&templateid=0&reportid=" +
                                data.info.maintainReport.Id +
                                "&carno=" +
                                carno +
                                "&iswx=" +
                                (data.info.wxopenid == "" ? 0 : 1)
                ...
                var txtmodelyear =
                    data.info.yearModel == null ? "" : data.info.yearModel;
                ...
                if (
                    ...
                    $(".fwadd_car dd")
                        ...
                        .css("background", "none")
                        .find("font")
                        .attr("brand", data.info.brandId)
                        .attr("line", data.info.lineId)
                        .attr("modelyear", data.info.modelYear)
                        .attr("model", data.info.modelId)
                        .attr("brandname", txtbrand)
                        .attr("linename", txtline)
                        .attr("modelyearname", txtmodelyear)
                        .attr("modelname", txtmodel)
                        .attr("vin", vin)
                        .html(
                            txtbrand + "  " + txtline + "  " + txtmodelyear + "  " + txtmodel
                        );
                }
            }
        }
            ...
            if (data.result == "T") {
                ...
                if (data.timespackage.length > 0) {
                    for (var i = 0; i < data.timespackage.length; i++) {
                        $("#timespackage").append(
                            '<tr labourprojectid="' +
                            (data.timespackage[i].LabourProjectId == null
                                ? 0
                                : data.timespackage[i].LabourProjectId) +
                            '" stockpartsid="' +
                            (data.timespackage[i].StockPartsId == null
                                ? 0
                                : data.timespackage[i].StockPartsId) +
                            '" remaindtimes="' +
                            data.timespackage[i].RemaindTimes +
                            '">' +
                            '<td align="left">' +
                            data.timespackage[i].Name + "(" + (data.timespackage[i].ExpirationDate == null ? "长期有效" : data.timespackage[i].ExpirationDate.ToString("yyyy-MM-dd")) + ")" +
                            "</td>" +
                            '<td  width="80" align="center" class="color999">' +
                            (data.timespackage[i].LabourProjectId != null
                                ? "工时"
                                : data.timespackage[i].StockPartsId != null ? "配件" : "") +
                            "</td>" +
                            '<td  width="80" align="center">' +
                            (data.timespackage[i].IsLimitTimes == 0
                                ? ""
                                : data.timespackage[i].TotalTimes -
                                data.timespackage[i].RemaindTimes) +
                            "</td>" +
                            '<td width="100" align="center"><span style="color: #32b400;">' +
                            (data.timespackage[i].IsLimitTimes != null &&
                            data.timespackage[i].IsLimitTimes == 0
                                ? "不限次数"
                                : data.timespackage[i].RemaindTimes) +
                            "</span></td>" +
                            '<td width="100" align="center"><span style="color: #32b400;">' +
                            (data.timespackage[i].LabourProjectId != null &&
                            (data.timespackage[i].ExpirationDate == null || comparetime(data.timespackage[i].ExpirationDate.ToString("yyyy/MM/dd")))
                            &&
                            (data.timespackage[i].IsLimitTimes == 0 ||
                                data.timespackage[i].RemaindTimes != 0)
                                ? "<a href='javascript:;'lang='" +
                                data.timespackage[i].LabourProjectId +
                                "' total='" +
                                data.timespackage[i].ProjectPrice +
                                "' salesname='销售员' salesid=''operatername='施工员'  operaterid='' typename='保养' qty='1' price='" +
                                data.timespackage[i].ProjectPrice +
                                "' vipprice='" +
                                data.timespackage[i].VipPrice +
                                "'  servicetypeid='" +
                                data.timespackage[i].ServiceTypeId +
                                "'    discount='100' name='" +
                                data.timespackage[i].Name +
                                "' itemid='" +
                                data.timespackage[i].LabourProjectId +
                                "'  class='addtcxm'><font></font></a>"
                                : "") +
                            "</span></td>" +
                            "</tr>"
                        );
                    ...
                    $("#timespackage").append(
                        '<tr><td colspan="4" align="center" style="height: 360px; border: 0; color: #999; font-size: 32px;">暂无次卡套餐项目</td> </tr>'
                    );
                }
                $("#textpackagelist").empty();
                if (data.textpackage.length > 0) {
                    for (var i = 0; i < data.textpackage.length; i++) {
                        $("#textpackagelist").append(
                            "<tr>" +
                            '<td align="left"><b>剩余</b>&nbsp;' +
                            (data.textpackage[i].Comment == null
                                ? ""
                                : data.textpackage[i].Comment) +
                            "</td>" +
                            "</tr>"
                        );
                    }
                    ...
                    $("#textpackagelist").append(
                        '<tr> <td colspan="4" align="center" style="height: 320px; border: 0; color: #999; font-size: 32px;">暂无文本套餐项目</td></tr>'
                    );
                }
                $("#vipcardlist").empty();
                if (data.card.length > 0) {
                    for (var i = 0; i < data.card.length; i++) {
                        $("#vipcardlist").append(
                            ...
                            data.card[i].CardNo +
                            "<br />(" +
                            data.card[i].TemplateName +
                            ")</td>" +
                            '<td width="150" align="center" >' +
                            ChangeDateFormat(data.card[i].OpenCardTime) +
                            "</td>" +
                            '<td width="130" align="center">' +
                            (data.card[i].ExpirationDate == null
                                ? "长期有效"
                                : ChangeDateFormat(data.card[i].ExpirationDate)) +
                            "</td>" +
                            '<td width="120" align="center"><span style="color: #ff6600;">¥' +
                            data.card[i].Amount +
                            "</span></td>" +
                            "</tr>"
                        );
                    ...
                    $("#vipcardlist").append(
                        '<tr><td colspan="4" align="center" style="height: 320px; border: 0; color: #999; font-size: 32px;">暂无储值卡</td> </tr>'
                    );
                }
            ...
            if (data.result == "T") {
                $("#pending .mask_btn a")
                    ...
                    .attr(
                        ...
                        service_recordlist +
                        "?start=" +
                        data.data.CreateTime.ToString("yyyy-MM-01") +
                        "&end=" +
                        serviceend +
                        "&status=" +
                        servicestatusvule +
...
function assemblyhtml(wid, randomid, total, salesname, salesid, operatername, operaterid, typename, name, qty, price, vipprice, servicetypeid, discount) {
    ...
    var htmlsb = "<div itemid='" + randomid + "' wid='" + wid + "' class='workhour'><dl f='working'><dt name='" + name + "' ><font defaultprice='" + defaultprice + "' price='" + price + "' vipprice='" + vipprice + "' qty='" + qty + "' discount='" + discount + "'>" + price + " * " + qty + (discount == 100 ? "" : "*<font style='color:#ff6600;'>" + discount + "折</font>") + "</font>" + substringname(name, 20) + "</dt>";
